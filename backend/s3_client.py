...
def ensure_bucket_exists():
    ...
    try:
        ...
        if not client.bucket_exists(config.MINIO_BUCKET):
            ...
            # Убеждаемся, что bucket имеет публичный доступ
            try:
                set_bucket_public_policy()
...
def set_bucket_public_policy():
    """Настраивает bucket policy для публичного доступа на чтение"""
    try:
        client = get_minio_client()
        # Bucket policy для публичного доступа на чтение
        policy = {
            "Version": "2012-10-17",
            "Statement": [
                {
...
def upload_photo(file_data: bytes, file_extension: str = "jpg") -> str:
    """
    Загружает фото в MinIO и возвращает URL для просмотра
    
    Args:
        file_data: Бинарные данные файла
        file_extension: Расширение файла (jpg, png, etc.)
    
    Returns:
        URL для просмотра фото
    """
    try:
        client = get_minio_client()
        ...
        # Генерируем простой публичный URL
...
def get_photo_url(object_name: str) -> str:
    """
    Получает URL для просмотра фото по имени объекта
